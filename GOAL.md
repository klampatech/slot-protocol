...
> Living spec for the game. Updated as features ship. Source of truth: `index.html` (single-file build, ~7700 lines as of current HEAD). All 9 payloads wired end-to-end, 6/6 daily modifiers via `modActive(id)`, 17 achievements, 11 peg types, 8 slot types, Phase 4 audio (Web Audio SFX + 3 active HTML5 BGM tracks: title / gameplay / ending — the `connection-lost.mp3` and `slot-machine.mp3` files are no longer in rotation), in-game tooltips for every peg + slot anchored to the mouse, **+ Phase 8a/8b/8c payload tooltips + dynamic slot cap (2 base, +1 every 10 floors) + consume-all-on-drop + dedup** (see Change Log). **+ Phase 9: slot-selector UI refresh + drag-and-drop alongside tap** (see Change Log). **+ Phase 10: settings volume sliders (SFX + BGM per-channel 0..1, replacing the on/off toggles)** (see Change Log). **+ Phase 11: mobile compatibility pass — layout-vs-visual fix (board no longer clips on phone viewports) + slot-selector tap-to-swap + drag fixes (first-tap no longer cancels its own selection) + new mobile test suite (Playwright device emulation across Pixel 7 / iPhone 13 / iPhone SE / iPad Mini)** (see Change Log). **+ Phase 11b: main-board glow border on narrow viewports — 15px horizontal body padding on viewports <= 600px + `scaleToFit` reads body padding + mobile test for glow room** (see Change Log). **+ Phase 12: SFX overhaul — per-peg-type hit sounds (11 types) + per-payload drop + activation sounds (9 types) + combo escalation layers + frenzy synth stab + slot machine layered reel sounds + jackpot multi-stage arc (chime → drop → sparkle) + near-miss sad trombone + slot selector 5-sound feedback + full audio/visual coupling + shared feedback delay + peak compressor** (see Change Log). Phase 4.5 economy-balancing pass complete (PR1 + A through Q): credit double-dip fixed (5% of floor-delta), peg-target rescaled (`12 + fl*1.5`), per-peg floor-scaled scoring, payload costs rebalanced twice (50–120 → 80–220, luxury tier), unique slot-pool with EMPTY excluded, daily-mod hard-tier dedup, chain timer +20%, explosive-peg scoring rewrite, jackpot 2/3 partial match, slot-machine result banner + reel-state cleanup, continuous BGM through runs, "CONNECTION LOST" end screen. All Known Gaps closed. **Phases 5 / 5b / 6 / 7 / 7b / 7c / 8a / 8b / 8c / 9 / 10 / 11 / 11b / 12 shipped**. **Post-feature code-review pass** shipped: 11 issues addressed (duplicate function, dead code, XSS via `textContent`, IIFE wrap, `var`→`const`/`let`, console-log removal, HUD DOM caching, timestamped render, canvas error boundary). **Test infra shipped**: 311 unit tests (Playwright, `tests/unit.js`, 24 sections) + 20 performance-validation tests + 16 screenshot tests + **54 mobile tests (Playwright device emulation, `tests/mobile.js`, 5 sections)** = **401 tests total**, all wired into CI (`.github/workflows/ci.yml`). `MULTIBALL_THRESHOLD` is reserved for the future meta-progression system (design decision, not a bug) — see Known Gaps section.

## Change Log

- **Phase 12b (SFX tuning pass)** — Three fixes after playtesting the Phase 12 audio overhaul. **1. Chain bar overflow bug** — AMPLIFY slot sets `GS.ct` to `C.CHAIN_T + 180` (= 216 frames), but the chain bar width was computed as `GS.ct / C.CHAIN_T * 100` = 583%, visually extending the bar way off screen. Fix: introduced `GS.ctMax` (tracks the highest GS.ct value this combo cycle) and the bar now computes `GS.ct / GS.ctMax * 100`, capped at 100%. `GS.ctMax` is reset on every combo reset (ball exit, chain timer expiry, overflow, floor clear, new game). The bar now correctly shows the percentage of the *current* chain timer, not the base timer. **2. Peg hit volumes too subtle** — the per-peg-type base volumes (NODE 0.15, CACHE 0.22, SEISMIC 0.35, etc.) were barely audible during gameplay. Bumped all volumes 40-60%: NODE 0.25/0.3, CACHE 0.34/0.38, TELEPORT 0.3/0.35, SEISMIC 0.45/0.55, EXPLOSIVE 0.6/0.65, DORMANT 0.28/0.32, ICE 0.3/0.35, FIBER 0.28/0.32, MIRROR 0.3/0.35, HONEYCOMB 0.3/0.35, OVERLOAD 0.35/0.38. The peak compressor handles the louder mix without clipping. **3. Combo pitch scaling more pronounced** — the NODE type already did `440 + combo * 60` Hz per step (a subtle 60Hz climb). Changed to `440 + combo * 80` Hz per step (wider interval per combo hit so each successive hit sounds distinctly HIGHER). The combo escalation sub-bass layer is now proportional to combo level (`0.15 + (cc-3) * 0.06`, capped at 0.45) instead of fixed at 0.1/0.15. The frenzy stab layer is also proportional to combo (`0.08 + cc * 0.02`, capped at 0.2) instead of fixed at 0.1. Combined, a cc=7 frenzy hit is now dramatically louder and higher-pitched than a cc=1 node hit. **Net effect** — `index.html` ~100 lines changed (per-peg-type volumes, dispatcher escalation, ctMax tracking). All 311 unit + 20 performance tests pass.

- **Phase 12 (SFX overhaul)** — Full audio engine overhaul. The game previously used a single `playPegHit(combo)` method for all 11 peg types (one square wave for everything) and a single `playSlotCollect` for slot effects. Payloads had no activation sounds (only daemon/cluster had any audio on launch). The slot machine was a 30ms 1000Hz square tick. The jackpot was a 4-note C-E-G-C arpeggio + sparkle. **New architecture.** The Audio object now has a layered internal structure: a peak compressor (`DynamicsCompressor`, -10dB / 12:1 ratio) prevents clipping when 3+ events overlap during high-combo frenzy bursts; a shared feedback delay (180ms, 25% feedback) adds a spatial reverb tail for bells/sweeps/jackpot while per-peg-type hits bypass it for clarity. New internal helpers: `_sub` (sub-bass thump via pitch-swept sine), `_bell` (5 inharmonic sine partials for metallic ring), `_coin` (3 staggered bell hits), `_kick` (150→40Hz body + highpass click transient), `_snare` (filtered noise + triangle body), `_glitch` (4 detuned squares staggered 5ms), `_sweep` (pitch-swept oscillator with optional bandpass filter). **Per-peg-type hit sounds (11 types).** Each peg type now has a distinct sonic identity matched to its visual + behavior. NODE = triangle ping. CACHE = two-note bell "ka-ching". TELEPORT = rising-then-falling bandpass sweep. SEISMIC = kick + noise + sub-bass. EXPLOSIVE = kick + multi-band noise (mid body + high crack + sizzle tail) + sub. DORMANT = rising sine sweep. ICE = high triangle + noise crack + bell ring. FIBER = building square tone (pitch increases with hit count; shatter + sub on 3rd hit). MIRROR = sharp square ping + bell ring. HONEYCOMB = building triangle tone (pitch increases with attractCount; 5-layer explosion on 5th hit: two bells + kick + noise + sub). OVERLOAD = descending sawtooth sweep + noise + sub. `playPegHitByType(pegType, combo, frenzy, extraData)` dispatches to the correct per-type method and adds two stacked layers: a combo escalation layer (sub-bass at cc≥3, louder sub at cc≥5, bell ping at cc=7) and a frenzy stab (a random square tone on every hit during GS.fa). **Per-payload sounds (9 types × 2 events).** Each payload has a `playPayloadDrop(name)` (called at ball launch in `dropBall`) and a `playPayloadActivate(name)` (called on the ball's first peg hit in `Peg.hit`). The drop is a brief signature sting identifying the payload; the activation is longer + more dramatic. Scrambler = sharp click → "WHOOP" + reverse noise + glitch. Trojan = descending chirp → 2 staggered bells + bonus chime. Worm = deep hum → phase-shift whoosh. Logic Bomb = rising C-E-G arpeggio + crackle → kick + bell + sub + bonus. Daemon = shield activation (existing) → low hum on activation. Ghost = whoosh + reverse → subtle phasing tick + glitch. Cluster = 3 quick "pew" sounds → tri-tone arpeggio on split (with "SPLIT!" float + magenta ring). Explosive = bomb-tick + preview explosion → full explosion on activation. Slowmo = time-warp stab + sub → rising-then-falling time-warp on activation. **Slot machine layered.** `playReelTick(reelIdx)` is now a 2-layer mechanical clack: noise transient ("metal-on-metal") + sine tick ("spring" resonance) with per-reel pitch variation (0=800Hz, 1=880Hz, 2=960Hz). `playReelStop(reelIdx, matched)` plays a bell ding for matched reels (the reel that helped the match) and a dull thud for the odd-one-out. The player can hear the build of tension as each reel lands. **Jackpot multi-stage arc.** `playJackpotChime()` = rising 6-note arpeggio (C-E-G-C-E-G) at 60ms intervals. `playJackpotDrop()` = sub + kick + two bells + 5 staggered coin clinks over 300ms. `playJackpotSparkle()` = 8 high-frequency twinkles at 80ms intervals. `playJackpot()` now sequences all 3 (chime → drop at 320ms → sparkle at 600ms) for a ~1s total arc. The spinJackpot 3/3 branch calls `playJackpot()` so the arc is synchronized with the coin burst visual celebration. **Near-miss sound.** `playNearMiss()` for the 2/3 partial: 2 quick bell pings (the matching reels) + sad-trombone descending sawtooth (the "so close" feeling). Replaces the old `playBonus()` which was the same sound as the jackpot fanfare and didn't convey the emotional arc. **Slot selector 5-sound feedback.** `playSelect` (ascending sine) on pool item selection, `playDeselect` (descending sine) on cancel, `playPlace` (noise + tone thunk) on drop, `playUnlock` (bell + sparkle) on unlock, `playSwap` (crossfade of two tones) on position swap. The old `playDragPickup` and `playDragDrop` are kept as aliases to `playSelect` and `playPlace` for backward compat. **Full audio/visual coupling.** Every new audio event has a matching visual hook. Per-peg-type particle bursts: `spawnPegBurstByType(type, x, y)` adds a distinct particle color + count + secondary color for each of the 11 peg types (CACHE gets 14 gold sparkles, EXPLOSIVE gets 24 red particles, MIRROR gets 12 white sparkles, etc.). First hit gets the full burst; repeat hits on the same peg get a reduced 4-particle ripple. Per-payload activation rings: `spawnPayloadRing(x, y, color, maxR)` adds an expanding colored ring at the activation point (logicbomb = gold 130px, scrambler = orange 90px, worm = green 100px, ghost = light-blue 100px, etc.). Combo escalation screen-edge glow: at cc≥3 a subtle gold glow appears along all 4 edges of the canvas; at cc≥5 it turns magenta; at cc=7 it turns white. Frenzy screen overlay: the existing "FRENZY x3!" text now has a pulsing magenta tint (sine-modulated, ts-driven) covering the full canvas. Max combo sting: the first time the player reaches cc=7 in a run, `playMaxCombo()` fires with a gold ring + bell + sub + bonus chime. **Net effect** — `index.html` +400 lines (Audio object rewrite ~350 lines, Peg.hit per-type dispatch + payload activation calls + spawnPegBurstByType call ~80 lines, dropBall per-payload drop sounds ~60 lines, cluster split sound in Ball.update ~10 lines, slot selector deselect/unlock/swap sounds ~10 lines, combo escalation + frenzy overlay render ~35 lines, jackpot multi-stage + reel-stop sounds + near-miss ~20 lines, comments ~35 lines; offset by the removal of the old Audio object body ~200 lines). `tests/unit.js` +250 lines (new "Phase 12: SFX System" section, 57 new asserts across 24 sections: DSP infrastructure, per-peg-type methods, per-payload methods, slot machine layered, jackpot multi-stage, slot selector, max combo, backward compat, visual coupling helpers, dispatcher routing, throttle, wire check). **All 311 unit + 20 performance + 16 screenshot + 54 mobile = 401 tests pass.** No game-logic changes — Phase 12 is purely an audio/visual engagement pass. **Smoke checks** (manual): every peg hit sounds distinct (NODE ping vs CACHE ka-ching vs EXPLOSIVE boom are immediately recognizable even during a frenzy), every payload has a signature sound on launch + activation (scrambler "WHOOP" on activation is distinctive, trojan "ping-ping" on activation is distinctive), the slot machine sounds like a real machine (the per-reel pitch variation makes the three reels audible), the jackpot celebration is a multi-second arc (chime → drop → sparkle, synced with 6 coin bursts), the near-miss "so close" feeling reads clearly (the sad-trombone descending after the 2-bell ping is instant recognition), the slot selector has distinct sounds for select/deselect/place/unlock/swap so the player can hear what just happened, the combo escalation screen-edge glow is visible at cc≥3 (gold) and cc≥5 (magenta) and the frenzy overlay pulses magenta, and the 311 unit tests all pass.

- **Phase 11b (main-board glow border on narrow viewports)** — Follow-up to the Phase 11 merge. Post-merge playtest on a Pixel 7a surfaced one remaining issue: the main peg board's `.board-active` glow border (2px solid cyan ring + 30px outer glow) is clipped on the left and right sides where the board meets the viewport edge. The overlays don't have this issue because their 2px border is a real `border` (inside the layout box, always visible) — only the outer glow is partially clipped. The main board's 2px ring is a `box-shadow: 0 0 0 2px` (outside the layout box, clipped by `overflow: hidden`), so the user saw only the inner half of the halo and a partial 2px ring. **1. Body horizontal padding on narrow viewports** — new `@media (max-width: 600px)` block in the body CSS adds 15px of horizontal padding (`calc(env(safe-area-inset-left, 0) + 15px)` on each side). The padding sits *outside* the board, giving the 30px CSS box-shadow (scaled to ~24-26px at the typical 0.6-0.86× scale on phones, so the half-blur is ~12-13px) enough room to render without being clipped by `overflow: hidden`. The 15px was chosen to cover all realistic phone viewports down to 320px wide (where the half-blur is ~10px, so 15px is comfortably enough). Desktop and tablet are unaffected — the media query excludes anything wider than 600px, and on those viewports the centered board already has hundreds of px of room on each side. **2. `scaleToFit` reads body padding** — without a corresponding JS change, the visual board would overflow the (now smaller) body content area and the padding would clip it (re-introducing the Phase 11-A clipping bug). `scaleToFit` now reads `getComputedStyle(body).paddingLeft` / `.paddingRight` / `.paddingTop` / `.paddingBottom` and subtracts them from the viewport dimensions before computing the scale. The scale is based on the body's content area, not the full viewport, so the visual board fits exactly inside the padded region. Desktop behavior is unchanged (body padding is just the safe-area-inset, which is 0 on most desktops). **3. Mobile test for glow room** — `tests/mobile.js` Layout / Scaling section now includes a regression check on viewports <= 600px: the scaler must have at least 14px of horizontal margin on each side (1px tolerance for sub-pixel rounding). Without the 11b fix, the scaler would be at left=0 and right=vpW (touching the viewport edges), and the new assertions would fail with the actual margin numbers in the detail. With the fix, all 6 new asserts pass (Pixel 7, iPhone 13, iPhone SE × 2 directions). iPad Mini is 768px wide so it doesn't trigger the media query and doesn't need the margin. **Net effect** — `index.html` +47 lines (CSS media query block ~20 lines, scaleToFit rewrite ~15 lines, comments ~12 lines). `tests/mobile.js` +11 lines (6 new asserts in the Layout / Scaling section). `package.json`, `svg-icons.js`, `.github/workflows/ci.yml` unchanged. **All 254 unit + 20 performance + 16 screenshot + 54 mobile = 344 tests pass.** No game-logic changes — Phase 11b is a CSS/JS sizing fix. **Smoke checks** (manual on Pixel 7a): the 2px cyan ring around the main board is now fully visible on the left and right (no clipping), the 30px outer glow halo extends the full 30px before fading (was clipped to the inner half pre-fix), the 2px ring + glow match the overlay borders in completeness. The board is ~7% smaller on a 412-wide viewport (382×557 instead of 412×600) — the trade-off for the fully visible glow.
...
- **Phases 5 / 5b / 6 / 7 / 7b / 7c / 8a / 8b / 8c / 9 / 10 / 11 / 11b / 12 all shipped** — See the detailed entries below. **Phase 12 is the most recent**: full SFX overhaul with per-peg-type hit sounds (11 types), per-payload drop + activation sounds (9 types), combo escalation layers, frenzy synth stab, slot machine layered reel sounds, jackpot multi-stage arc (chime → drop → sparkle), near-miss sad trombone, slot selector 5-sound feedback, full audio/visual coupling, shared feedback delay, and peak compressor. 57 new unit tests in `tests/unit.js` (311 total across 24 sections). See the Phase 12 change-log entry at the top of this file.
...
- **Test infrastructure shipped** — New `window.__TEST__` API in `index.html` exposes internals (GS, C, BOARD_TEMPLATES, `maxPayloads`, `renderPayloadSlots`, `renderSlotSelector`, `getSlotColor`, `setSfxVolume`, `setBgmVolume`, `applyVolumeSlider`, `Peg`, `Ball`, `spawnPayloadRing`, `spawnPegBurstByType`, etc.) for headless testing. `tests/unit.js` is the Playwright-driven unit suite: **311 asserts across 24 sections** (Phase 12 added 1 new section — "Phase 12: SFX System" with 57 asserts covering DSP infrastructure, per-peg-type methods, per-payload methods, slot machine layered, jackpot multi-stage, slot selector feedback, max combo sting, backward compat, visual coupling helpers, dispatcher routing, throttle, and wire check). Existing sections: state init, constants, board generation, physics, scoring, slot system, peg types, achievements, daily challenge, payloads, persistence, tooltip data, screen management, game start/reset, jackpot growth, board templates, contrast mode, audio engine, volume sliders, volume migration, Phase 9 slot-selector UI, render-loop regression, and a render-loop regression guard for the `updateFX/updateVisualEffects` `(dt, ts)` arity. `tests/performance-validation.js` has 21 asserts (memory-leak detection across 50 simulated floors, FPS validation under normal/varied/heavy load, multiball physics desync detection, localStorage overflow handling, payload-injection enforcement). `tests/screenshot-suite.js` captures **16 PNGs** in `tests/screenshots/` with per-screenshot visual assertions (overlay-visible + peg-band pixel counts + HUD text + render-loop `pageerror` tracking); `06-slot-selector.png` was regenerated for the Phase 9 visual refresh. `package.json` exposes `npm test` (performance), `npm run test:unit` (Playwright unit), `npm run test:screenshot` (Playwright screenshots). Expected pass on a clean checkout: 21/21 performance + 311/311 unit + 16/16 screenshot.
...
- **`index.html` is ~7700 lines as of HEAD.** Bumped from 5680 at Phase 4.5-Q → ~6250 at Phase 7 → 6331 at Phases 8a/8b → 6529 at Phase 8c → ~6769 at Phase 9 → 7164 at Phase 10 → ~7700 at Phase 12 (SFX overhaul, +400 net: Audio object rewrite + per-peg-type dispatch + per-payload sounds + visual coupling). `svg-icons.js` is 208 lines (unchanged from Phase 10). `tests/unit.js` is ~1573 lines (was 1323; +250 for the Phase 12 SFX tests).
...
## Phase 11b — Main-Board Glow Border on Narrow Viewports

> Post-Phase 11 follow-up. User merged PR #7 and tested on a real Pixel 7a. Everything looked good except one detail: the main peg board's `.board-active` glow border (2px solid cyan ring + 30px outer glow) was clipped on the left and right where the board met the viewport edge. Detail in the Phase 11b change-log entry at the top of this file.

### The bug

The main board's `.board-active` glow is defined as `box-shadow: 0 0 0 2px #00fff2, 0 0 30px rgba(0,255,242,0.3)` on `#game-container`. Both box-shadows are *outside* the layout box. The board (480×700 layout, scaled to fill the viewport edge-to-edge after the Phase 11-A layout-vs-visual fix) is at the very edge of the body. The body's `overflow: hidden` clips the box-shadow on the left and right where the board meets the viewport edge. The user saw:
- The 2px solid ring as a thin partial line (~1px) on each side — the rest was clipped.
- The 30px outer glow as only the *inner half* of the halo (the part that fades into the board) — the outer half (which would extend beyond the viewport) was clipped.

The overlays (menu, settings, slot-selector, etc.) don't have this issue because their 2px border is a real `border` (inside the layout box, always visible) — only their outer 30px glow is partially clipped, and the user is OK with that (they said the overlays look fine). The asymmetry was the main board's 2px "border" being outside instead of inside.

### The fix

**1. Body horizontal padding on narrow viewports.** New `@media (max-width: 600px)` block adds 15px horizontal padding to the body (`padding-left: calc(env(safe-area-inset-left, 0) + 15px)` and the mirror on the right). The 15px is added on top of the safe-area-inset, so on a notched phone in landscape the safe-area still wins. The 15px provides enough room for the 30px CSS box-shadow (scaled to ~24-26px at the typical 0.6-0.86× phone scale, so the half-blur is ~12-13px). Desktop and tablet (>600px) are unaffected — the centered board already has hundreds of px of room on each side.

**2. `scaleToFit` reads body padding.** Without a corresponding JS change, the visual board would overflow the (now smaller) body content area and the padding would clip it (re-introducing the Phase 11-A clipping bug). `scaleToFit` now reads `getComputedStyle(body).paddingLeft` / `.paddingRight` / `.paddingTop` / `.paddingBottom` and subtracts them from the viewport dimensions before computing the scale. The scale is based on the body's content area, not the full viewport, so the visual board fits exactly inside the padded region. Desktop behavior is unchanged (body padding is just the safe-area-inset, which is 0 on most desktops).

**3. Mobile test for glow room.** `tests/mobile.js` Layout / Scaling section now includes a regression check on viewports <= 600px: the scaler must have at least 14px of horizontal margin on each side (1px tolerance for sub-pixel rounding). Without the 11b fix, the scaler would be at left=0 and right=vpW (touching the viewport edges), and the new assertions would fail with the actual margin numbers in the detail. With the fix, all 6 new asserts pass (Pixel 7, iPhone 13, iPhone SE × 2 directions). iPad Mini is 768px wide so it doesn't trigger the media query and doesn't need the margin.

### Net effect

- **`index.html`**: +47 lines (CSS media query block ~20 lines, scaleToFit rewrite ~15 lines, comments ~12 lines). 7338 → 7385 lines.
- **`tests/mobile.js`**: +11 lines (6 new asserts in the Layout / Scaling section).
- **`package.json`**, **`svg-icons.js`**, **`.github/workflows/ci.yml`**: unchanged.
- **All 254 unit + 20 performance + 16 screenshot + 54 mobile = 344 tests pass.**

### Smoke checks (manual on Pixel 7a)

- The 2px cyan ring around the main board is now fully visible on the left and right (no clipping).
- The 30px outer glow halo extends the full 30px before fading (was clipped to the inner half pre-fix).
- The 2px ring + glow match the overlay borders in completeness.
- The board is ~7% smaller on a 412-wide viewport (382×557 instead of 412×600) — the trade-off for the fully visible glow.

### Design iterations (rejected)

- **Use `box-shadow: inset 0 0 0 2px` on the main board** to make the 2px ring always visible (like the overlays' inside `border`) and skip the padding. Rejected: the 30px outer glow would still be partially clipped, and the user explicitly asked for the "entire glow border" to be visible. The inset 2px alone wouldn't address the glow clipping.
- **Use a pseudo-element on the body to render the glow with explicit room.** Rejected: a body-level pseudo-element would need to be positioned behind the #scaler and sized to include the glow halo, which is complex and would interfere with the body's flex centering. The 15px padding is simpler and gives the same visual result.
- **Use `outline` on `#game-container` with `outline-offset: -2px`** to draw the 2px inside the box. Rejected: `outline` doesn't render glow (just a solid line), so the 30px outer halo would still be clipped. The user's complaint was about the entire glow border, not just the 2px ring.
- **Increase the body padding to 20px or 25px** to be extra safe. Rejected: 15px is enough for all realistic phone viewports (the half-blur at the smallest realistic scale is ~10px on a 320-wide iPhone SE), and a smaller padding means a larger board. 15px is the minimum that gives full glow visibility on every device from 320px to 600px wide, and the trade-off (board is ~7% smaller) is at the lower end of the acceptable range.
- **Add the padding globally (not media-query-gated).** Rejected: on desktop the board is centered with hundreds of px of room, and shrinking it by 15px on each side would be a pointless regression. The media query is the right scope.
...
### Mobile Compatibility — Glow Border (Phase 11b)
- [x] Main board's `.board-active` glow border (2px cyan ring + 30px outer glow) is fully visible on narrow viewports — no left/right clipping. New `@media (max-width: 600px)` block adds 15px horizontal body padding (on top of safe-area-inset) so the box-shadow has room to render.
- [x] `scaleToFit` reads the computed body padding and subtracts it from the viewport dimensions, so the visual board fits exactly inside the padded region (no overflow re-introducing the Phase 11-A clipping bug).
- [x] Desktop and tablet (viewport > 600px) are unaffected — the media query excludes them and the centered board already has hundreds of px of room on each side.
- [x] Mobile test suite now checks the scaler has at least 14px horizontal margin on viewports <= 600px (6 new asserts across Pixel 7, iPhone 13, iPhone SE). Regression guard for the 11b fix.
