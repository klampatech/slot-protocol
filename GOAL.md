...
> Living spec for the game. Updated as features ship. Source of truth: `index.html` (single-file build, 4931 lines as of Phase 2c). All 9 payloads wired end-to-end, 6/6 daily modifiers via `modActive(id)`, 17 achievements, 11 peg types, 8 slot types, Phase 4 audio (Web Audio SFX + HTML5 BGM), in-game tooltips for every peg + slot anchored to the mouse. 3 Known Gaps remain closed; the 4th (`MULTIBALL_THRESHOLD`) is a design question, not a bug.
...
- **Open Known Gap (1)**: `MULTIBALL_THRESHOLD` constant defined but never enforced — multiball is opt-in via settings toggle only. This is a design question, not a bug.
...
- Multiball: MAX_BALLS=3, MULTIBALL_THRESHOLD=7 (combo to unlock multiball, currently not enforced — toggle is opt-in)
...
- [ ] **`MULTIBALL_THRESHOLD`**: constant defined but never enforced (multiball is opt-in only, never auto-unlocks at combo 7)
